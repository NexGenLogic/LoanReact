...
import { updateLoan } from "@/utils/loan_services";
import showSwal from "@/mixins/showSwal.js";
import EditLoanDocuments from "./EditLoanDocuments.vue"

export default {
    name: "EditViewLoanComponent",
    components: {
        MaterialInput,
        MaterialButton,
        AppFormNew,
    ...
    watch: {
        loan: {
            ...
            deep: true       // If nested values of loan might change
        }
    },
    mounted() {
        if (this.loan?.image) {
            // Ensure you construct the full URL to the image
            this.previewImage = this.loan.image;
        } else {
            this.previewImage = "";
        }
    },
        ...
        initialValues() {
            return {
                user: {
                    ...
                    phone_number: this.loan?.phone_number || '',
                    address_line1: this.loan?.address_line1 || '',
                    address_line2: this.loan?.address_line2 || '',
                    city: this.loan?.city || '',
                    state: this.loan?.state || '',
                    postal_code: this.loan?.postal_code || '',
                    country: this.loan?.country || '',
    ...
    methods: {
        ...
        async handleSubmit(values) {
            ...
            } else if (!this.previewImage && this.loan.image) {
            ...
            try {
                await updateLoan(this.loan.id, formData);
                showSwal.methods.showSwal({
                    type: "success",
                    message: "Store updated successfully!",
                    width: 500
                });
                this.$emit('loan-updated');

            ...
            <tr v-for="(each_loan, index) in loan" :key="index">
                    ...
                    <div class="d-flex px-2 py-1">
                        <div class="d-flex flex-column justify-content-center w-100">
                            ...
                            <div class="loan-meta d-flex flex-column gap-1">
                                <div v-if="each_loan.created_at" 
                                    >
                                    
                                    <span class="pill-text">{{ new Date(each_loan.created_at).toLocaleDateString() }}</span>
                                </div>
                ...
                <td class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                    <div class="d-flex flex-column justify-content-center">
                        <h6 class="mb-0 text-sm">{{ each_loan.updater_name || 'N/A' }}</h6>
                    </div>
                </td>
                <td class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                    <div class="d-flex flex-column justify-content-center">
                        <h6 class="mb-0 text-sm">{{ each_loan.type ? each_loan.type.replace(/_/g, ' ') : '' }}</h6>
                    </div>
                </td>
                <td class="align-middle  text-sm">
                    <span class="badge badge-sm" :class="updateStatusBadges[each_loan.status] || 'bg-gradient-secondary'">
                                {{  each_loan.status }}
                    </span>
                </td>
                <td class="align-middle">
                    <div class="d-flex gap-4">
                        <a href="javascript:;" class="text-secondary font-weight-bold text-xs"
                            @click="view_loan(each_loan)" title="View">
                            <i class="material-icons-round opacity-10 fs-5">visibility</i>
                        </a>
                        
                        <a href="javascript:;" class="text-secondary font-weight-bold text-xs "
                            @click="tryToDelete(each_loan)" title="Delete" v-if="is_allowed('loan_updates','delete')">
...
import { fetchLoanUpdates, deleteLoanUpdate } from "@/utils/loan_services";
import { isAdmin,is_allowed  } from "@/utils/api_services";
...
export default {
    ...
    methods: {
        ...
        async tryToDelete(item) {
            this.selectedLoanUpdate = item;
            let confirmed = await showSwal.methods.showSwalConfirmationDelete();
            if (confirmed.isConfirmed) {
                try {
                    await deleteLoanUpdate(this.selectedLoanUpdate.id);
                    showSwal.methods.showSwal({
                        type: "success",
        ...
        view_loan(loan) {
            this.$router.push({ name: 'View Updated Loan', params: { loanId: loan.id } });
        },
        async loadLoans(page = 1) {
            ...
            try {
                ...
                let response = await fetchLoanUpdates(payload);

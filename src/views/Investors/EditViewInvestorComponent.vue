    ...
    <AppFormNew role="form" :validation-schema="schema" @submit="handleSubmit" :initial-values="initialValues"
        ...
        <div class="multisteps-form__panel border-radius-xl bg-white" data-animation="FadeIn">
            <div class="multisteps-form__content">
                ...
                <div class="button-row d-flex mt-4"  v-if="is_allowed('investors','update')">
...
import { updateInvestor } from "@/utils/investor_services";
import showSwal from "@/mixins/showSwal.js";
import EditInvestorDocuments from "./EditInvestorDocuments.vue"
import { is_allowed } from "@/utils/api_services"; 

export default {
    name: "EditViewInvestorComponent",
    components: {
        MaterialInput,
        MaterialButton,
        AppFormNew,
    ...
    watch: {
        investor: {
            ...
            deep: true       // If nested values of investor might change
        }
    },
    mounted() {
        if (this.investor?.image) {
            // Ensure you construct the full URL to the image
            this.previewImage = this.investor.image;
        } else {
            this.previewImage = "";
        }
    },
        ...
        initialValues() {
            return {
                user: {
                    ...
                    phone_number: this.investor?.phone_number || '',
                    address_line1: this.investor?.address_line1 || '',
                    address_line2: this.investor?.address_line2 || '',
                    city: this.investor?.city || '',
                    state: this.investor?.state || '',
                    postal_code: this.investor?.postal_code || '',
                    country: this.investor?.country || '',
    ...
    methods: {
        ...
        async handleSubmit(values) {
            ...
            } else if (!this.previewImage && this.investor.image) {
            ...
            try {
                await updateInvestor(this.investor.id, formData);
                showSwal.methods.showSwal({
                    type: "success",
                    message: "Store updated successfully!",
                    width: 500
                });
                this.$emit('investor-updated');

            } catch (error) {
                console.error("Error creating investor:", error);
                let errorMessage = "Failed to update investor. Please try again.";
                if (error.html) {

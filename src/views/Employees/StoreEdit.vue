...
export default {
    ...
    setup(props) { // Using setup function for better reactivity with props
        ...
        // Function to initialize/reset formValues based on the store prop
        const initializeForm = (storeData) => {
            formValues.value = {
                name: storeData.name || '',
        ...
        initializeForm(props.store);

        // Watch for changes in the store prop to re-initialize if needed
        // This is important if the `store` prop can change after the component is mounted
        // and you want the form to reflect that.
        watch(() => props.store, (newStoreData) => {
            initializeForm(newStoreData);
        }, { deep: true }); // Use deep watch if store object structure might change internally
        ...
        const handleSubmit = (values, { resetForm }) => {
            ...
            // Example: this.$store.dispatch('updateStore', values);
    ...
    //    store: {
    //        handler(newStore) {
    //            // If you need to imperatively update the form if `store` changes

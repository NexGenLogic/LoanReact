<template>
  <div class="modal fade" ref="customerStatusModal" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-body" v-if="customer">
          <p>
            ...
            <span class="text-sm">Current Status: 
              <span :class="`badge bg-gradient-${statusBadges[customer.status]}`">{{ statusLabels[customer.status] || customer.status }}</span>
            </span>
          </p>
          ...
          <form @submit.prevent="submitStatusChange" class="mt-4">
            ...
            <div class="row align-items-center mb-3">
              <div class="col-4">
                <label for="customer-status" class="col-form-label fw-bold">Status:</label>
              </div>
              <div class="col-8">
                <select id="customer-status" class="form-select" v-model="newStatus" required>
...
import { updateCustomerStatus } from "@/utils/customer_services"; 
...
export default {
  ...
  mounted() {
    this.modalInstance = new Modal(this.$refs.customerStatusModal);
  },
  methods: {
    show() {
      // *** THE FIX IS HERE ***
      // This ensures the form is reset with the current customer's data
      // every single time the modal is opened.
      if (this.customer) {
        this.newStatus = this.customer.status;
    ...
    async submitStatusChange() {
      ...
      try {
        ...
        await updateCustomerStatus(this.customer.id, formData);

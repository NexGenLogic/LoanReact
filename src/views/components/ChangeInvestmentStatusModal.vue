<template>
  ...
  <BaseModal ref="statusModal" modal-id="changeStatusModal" size="sm">
    <template #header>
      <h5 class="modal-title">Change Investment Status</h5>
      <button type="button" class="btn-close" @click="close" aria-label="Close"></button>
    </template>

    <!-- The form now uses Bootstrap's grid system -->
    <form v-if="investment" @submit.prevent="saveStatus">
      <!-- Row 1: Status Dropdown -->
      <div class="row align-items-center mb-3">
        <div class="col-4">
          <label for="investment-status" class="col-form-label fw-bold">Status:</label>
        </div>
        <div class="col-8">
          <select id="investment-status" class="form-select" v-model="formData.status" required>
...
import { updateInvestmentStatus } from '@/utils/investment_services';
...
export default {
  ...
  watch: {
    investment(newInvestment) {
      if (newInvestment) {
        this.formData.status = newInvestment.status;
        this.formData.notes = '';
      }
    }
  },
  methods: {
    ...
    async saveStatus() {
      ...
      this.loading = true;
      try {
        await updateInvestmentStatus(this.investment.id, this.formData);
